...
export function Camp_Dash(){
    ...
    return(
        <div className="w-10/12 h-[85vh] flex justify-evenly flex-row items-center rounded-2xl">
            <div className="h-full w-4/12 bg-white rounded-2xl min- p-2 max-w-56 right-8 flex">
              <div className="flex flex-col justify-start items-center">
{/*Botoes*/}
            {botoes.map((botao) =>(
                <button key={botao.id} onClick={() => setActiveId(botao.id)}  className={`flex flex-row items-center text-left pr-14 border-none text-xl h-10 m-1 w-full rounded-xl transition-all duration-300 ${
                    activeId === botao.id
                      ...
                      : "bg-white text-primary-700 hover:bg-primary-300 hover:text-white"
                        ...
                        >
                        {botao.img}
                        <span className="ml-2">{botao.nome}</span></button>
              ))}               

              </div>
            </div>

            <div className="h-full w-9/12 bg-white rounded-2xl">
                 {/*Como faz essporra???  esssa e a div que quero colocar o return do ativo(id)*/}
                 {renderContent()}
            </div>
