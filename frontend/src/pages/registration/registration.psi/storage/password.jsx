...
export function Password({ passwordError, confirmPasswordError }) {
    ...
    const [VisibleConfirmarSenha, SetVisibleConfirmarSenha] = useState(false)
    const [FeedbackSenha, SetFeedbackSenha] = useState('')
    const [CorFeedbackSenha, CorSetFeedbackSenha] = useState('text-primary-700')

    const VerificaSenha = (e) => {
        const senha = e.target.value
        setPassword(senha)

        const verificar = zxcvbn(senha)

        if (senha == ""){
            SetFeedbackSenha("")
            CorSetFeedbackSenha('text-primary-700')
            return
        }

        switch (verificar.score) {
            case 0:
                SetFeedbackSenha('Senha muito fraca')
                CorSetFeedbackSenha("text-red-800")
                break
            case 1:
                SetFeedbackSenha('Senha fraca')
                CorSetFeedbackSenha("text-red-600")
                break
            case 2:
                SetFeedbackSenha('Senha media')
                CorSetFeedbackSenha("text-orange-500")
                break
            case 3:
                SetFeedbackSenha('Senha forte')
                CorSetFeedbackSenha("text-green-500")
                break
            case 4:
                SetFeedbackSenha('Senha muito forte')
                CorSetFeedbackSenha("text-green-800")
                break
            default:
                SetFeedbackSenha('Senha inválida')
                CorSetFeedbackSenha("text-grey-800")
        }

        if (verificar.score >= 3){
            console.log(senha)
            return
        } else {
            e.prevent.Default()
    ...
    const ControladorVisibilidadeConfirmarSenha = () => {
        SetVisibleConfirmarSenha(!VisibleConfirmarSenha)
    }
    ...
    return (
        <div className="flex flex-col md:flex-row gap-5 p-4 justify-between items-start max-md:mr-2 max-md:flex-row max-md:h-fit max-md:p-0 max-md:mt-10 max-xl:p-0 max-md:w-[90%]">
            <div className="flex flex-col md:w-2/5">
                ...
                <p className={`font-poppins max-md:text-[12px] md:text-[14px] absolute translate-y-14 ${CorFeedbackSenha}`}>{FeedbackSenha}</p>
            </div>
            <div className="flex flex-col md:w-2/5">
                ...
                <div className='relative'>
                <input
                    type={VisibleConfirmarSenha ? 'text' : 'password'}
                ...
                <button
                        type='button'
                        onClick={ControladorVisibilidadeConfirmarSenha}
                        className='absolute right-2 duration-500 top-[5px]'>
                        {VisibleConfirmarSenha ? (
                            <svg className="w-6 h-6 text-slate-900" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M2.42012 12.7132C2.28394 12.4975 2.21584 12.3897 2.17772 12.2234C2.14909 12.0985 2.14909 11.9015 2.17772 11.7766C2.21584 11.6103 2.28394 11.5025 2.42012 11.2868C3.54553 9.50484 6.8954 5 12.0004 5C17.1054 5 20.4553 9.50484 21.5807 11.2868C21.7169 11.5025 21.785 11.6103 21.8231 11.7766C21.8517 11.9015 21.8517 12.0985 21.8231 12.2234C21.785 12.3897 21.7169 12.4975 21.5807 12.7132C20.4553 14.4952 17.1054 19 12.0004 19C6.8954 19 3.54553 14.4952 2.42012 12.7132Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M12.0004 15C13.6573 15 15.0004 13.6569 15.0004 12C15.0004 10.3431 13.6573 9 12.0004 9C10.3435 9 9.0004 10.3431 9.0004 12C9.0004 13.6569 10.3435 15 12.0004 15Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
